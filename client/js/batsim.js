...
function beginBattle (date,namea,nameb,A,B,back,heroA,heroB,tinfo,next,promA,promB,grid,vals) {
    ...
    if (nextLeftTurn.self==undefined) nextLeftTurn.self=getTurnData(left.setup.length,right.setup.length);
    if (nextLeftTurn.other==undefined) nextLeftTurn.other=getTurnData(right.setup.length,left.setup.length);
    if (nextRightTurn.self==undefined) nextRightTurn.self=getTurnData(right.setup.length,left.setup.length);
    if (nextRightTurn.other==undefined) nextRightTurn.other=getTurnData(left.setup.length,right.setup.length);

    if (nextLeftTurn.self!==undefined) doTurn(left,right,nextLeftTurn.self,nextLeftTurn.other,0);
    if (nextLeftTurn.other!==undefined) doTurn(right,left,nextLeftTurn.other,nextLeftTurn.self,1);
    if (nextRightTurn.self!==undefined) doTurn(right,left,nextRightTurn.self,nextRightTurn.other,1);
    if (nextRightTurn.other!==undefined) doTurn(left,right,nextRightTurn.other,nextRightTurn.self,0);
    ...
    while (isAlive(left) && isAlive(right) && --turns) {
        ...
        while ((leftTurn.length || rightTurn.length) && --subturns) {
            ...
            killUnits(right,1,rightTurn);
            killUnits(left,0,leftTurn);
        }
        endTurn(left);
        endTurn(right);
    }
...
function spawnUnits(left,right) {
    ...
    for (var i=Math.max(left.setup.length,right.setup.length)-1; i>=0; --i) {
      if (i<right.setup.length && right.setup[i].extra!==undefined && right.setup[i].extra.length>0) {
          for (var j=0; j<right.setup[i].extra.length; j++) {
              gBattle.steps.push({
                  action:"EXTRA",
                  target:"other",
                  pos:i+1,
                  data:objCopy(right.setup[i].extra[j])
              });
          }
      }
    ...
    for (var i=Math.max(left.setup.length,right.setup.length)-1; i>=0; --i) {
      if (i<left.setup.length && left.setup[i].extra!==undefined && left.setup[i].extra.length>0) {
        for (var j=0; j<left.setup[i].extra.length; j++) {
            gBattle.steps.push({
                action:"EXTRA",
                target:"you",
                pos:i+1,
                data:objCopy(left.setup[i].extra[j])
            });
        }
    }
  }
...
function createUnits (userData,hero,prom,battleData,seed,grid,vals) {
    ...
    for (var i=userData.length-1; i>=0; --i) {
        ...
        if ([97,98,99,161,162,164,206,207,208,209].indexOf(hpos)!==-1) easter=true;
    }
    for (var i=userData.length-1; i>=0; --i) {
        if (userData[i]>=0) {
            var unitData = {
			...
			if (HERO[hpos].skill.type=="aposbonus") {
                var behind = 0;
                for (var j=i-1; j>=0; --j) if (userData[j]!=-1) ++behind;
                var val = Math.round(skillVal*Math.min(99,hero[hpos])*behind);
                stats.atk+=val;
                stats.hp+=val;
            }
            var unitData = {
            ...
            if (unitData.prom >= 6) {
                unitData.extra.push({
                    type: HERO[hpos].passive.type,
                    val: HERO[hpos].passive.value,
                });
                if (HERO[hpos].passive.type==9) {
                    unitData.hp=Math.round((1+HERO[hpos].passive.value)*unitData.hp);
                    unitData.mhp=unitData.hp;
                } 
            }
            if (unitData.skill.type=="payback") unitData.payback = 0;
            battleData.setup.push(unitData);
        ...
        if (grid!==undefined&&grid[i]!=-1&&userData[i]!=-1&&battleData.setup.length>0&&battleData.setup[battleData.setup.length-1].extra!==undefined) {
            battleData.setup[battleData.setup.length-1].extra.push({
                type: grid[i],
                val: vals[i],
            });
            if (grid[i]==9) {
...
function cleanTurn (turn,torem) {
    var mode = "buff";
    for (var param in turn[mode]) {
        if (Array.isArray(turn[mode][param])) {
            for (var i = 0; i < torem.length; ++i) {
                turn[mode][param].splice(torem[i],1);
            }
        }
    }
        ...
        if (A.setup[i].skill!==undefined) {
            ...
            if (skill.type=="paoe") {
                for (var j=0; j<B.setup.length; ++j) {
                    turn.atk.flatAoe[j]+=skillVal*lvlVal;
                }
            } else if (skill.type=="bhit") {
                var least = 0;
                for (var j=1; j<B.setup.length; ++j) {
                    if (B.setup[j].mhp<B.setup[least].mhp) least=j;
                }
                if (B.setup.length) {
                   turn.atk.flatAoe[least]+=Math.floor(skillVal*lvlVal);
                }
            } else if (skill.type=="flatlepre") {
                for (var j=0; j<B.setup.length; ++j) {
                    turn.atk.flatAoe[j]+=Math.max(0,B.setup.length-A.setup.length)*skillVal*Math.floor(lvlVal/9);
                }
...
function applyExtra(arr,turn) {
    ...
    */
    ...
    for (var i=0; i<arr.length; ++i) {
        for (var j=0;arr[i].extra!==undefined && j<arr[i].extra.length;++j){
            if (arr[i].extra[j]!==undefined) {
                if (arr[i].extra[j].type==0) {
                    turn.buff.affinity[i]+=arr[i].extra[j].val;
                } else if (arr[i].extra[j].type==1) {
                    turn.buff.angel[i]+=arr[i].extra[j].val;
                } else if (arr[i].extra[j].type==2) {
                    turn.buff.def[i]+=Math.round(turn.buff.def[i]*arr[i].extra[j].val);
                } else if (arr[i].extra[j].type==3 && i==0) {
                    arr[i].tatk=Math.round(arr[i].tatk*(1+arr[i].extra[j].val));
                } else if (arr[i].extra[j].type==5 && i==0) {
                    turn.buff.dpsmul+=arr[i].extra[j].val;
                } else if (arr[i].extra[j].type==6) {
                    turn.buff.heal[i]+=Math.round(turn.buff.heal[i]*arr[i].extra[j].val);
                } else if (arr[i].extra[j].type==7) {
                    turn.buff.sdefPerc[i]=Math.max(0,turn.buff.sdefPerc[i]-arr[i].extra[j].val);
                } 
            }
        } 
    }
        ...
        if (A.setup[i].skill!==undefined) {
            ...
            if (i==0) {
                if (skill.type=="buff") turn.buff.iAtkPerc[i]*=skillVal;
                    ...
                    for (var j=0; j<B.setup.length;++j) {
                        if (B.setup[j].id <= -2 && B.setup[j].type == "antireflect") {
                            var BskillVal = B.setup[j].skill.value;
                            if (B.setup[j].prom>=5) BskillVal = BskillVal + promoData[-2*1-B.setup[j].id].skill;
                            var antivalue = skillVal*(BskillVal*Math.min(99,B.setup[j].lvl)/B.setup[j].skill.target);
                            turn.atk.flatAoe[j] += (skillVal - antivalue);
                        } else turn.atk.flatAoe[j] += skillVal;
                    }
                } else if (skill.type=="elem") {
                    for (var j=0; j<B.setup.length;++j) {
                  ...
                  turn.onkill.absorb=skillVal;
                    ...
                    if (B.setup.length > 1) {
                        var lastUnitAlive = B.setup.length - 1;
                        turn.atk.damageFactor[lastUnitAlive]=turn.atk.damageFactor[0]*skillVal;
                        turn.atk.typeMul[lastUnitAlive]=turn.atk.typeMul[0];
                        if (B.setup.length > 2) {
                            turn.atk.damageFactor[lastUnitAlive-1]=turn.atk.damageFactor[lastUnitAlive]*skillVal;
                            turn.atk.typeMul[lastUnitAlive-1]=turn.atk.typeMul[lastUnitAlive];
                        }
                    }
                }
            } else {
            ...
            } else if (skill.type=="empow" && ((turnid%2)==0)) {
                turn.buff.iAtk[i]+=Math.round(skillVal*Math.floor(lvlVal/9)*(i));
                turn.buff.iHp[i]+=Math.round(skillVal*Math.floor(lvlVal/9)*(i));
            } else if (skill.type=="supershield" && turnid==1) {
                turn.buff.defPerc[i]+=skillVal*Math.floor(lvlVal/skill.target);
            }
            // all own units
            for (var j=0; j<A.setup.length;++j) {
                ...
                if (typeMatches) {
                    ...
                    if (skill.type=="buffdef" || skill.type=="buffboth") {
                        ...
                        turn.buff.def[j]+=Math.floor(lvlVal*skillVal);
                    }
                        ...
                        turn.buff.heal[j]+=Math.floor(lvlVal*skillVal);
                        ...
                        A.setup[j].tatk+=Math.floor(lvlVal*skillVal);
                    }
                }
            ...
            for (var j=0; j<B.setup.length;++j) {
                ...
                if (typeMatches) {
                    ...
                    if (skill.type=="buffall") turn.atk.flatAoe[j]+=Math.floor(lvlVal*skillVal);
                }
                if (skill.type=="guardian") turn.atk.flatAoe[j]+=Math.floor(skillVal*lvlVal/skill.target*1.5);
                else if (skill.type=="autobuffd") turn.atk.flatAoe[j]+=skillVal*Math.pow(2,Math.floor(turnid/skill.target));
                else if (skill.type=="autobuff") turn.atk.flatAoe[j]+=skillVal+turnid*skill.target;
                else if (skill.type=="combo") turn.atk.flatAoe[j]+=skillVal*A.setup[i].hp;
                else if (skill.type=="lullaby" && turnid<=skill.target) turn.atk.damageFactor[j]*=1+skillVal;
            }
    ...
    for (var i=0;i<D.setup.length;++i) {
        ...
        if (maxhp==i&&atk.anarchy2>0) {
            if (D.setup[i].id <= -2 && D.setup[i].skill.type == "antireflect" && D.setup.length > 0) {
                ...
                var antivalue = atk.anarchy2*(skillVal*Math.min(99,D.setup[i].lvl)/D.setup[i].skill.target);
                atk.flatAoe[i]+=Math.round(atk.anarchy2-antivalue);
        ...
        if (D.setup[i].skill!==undefined && D.setup[i].skill.type=="payback") D.setup[i].payback+=totalDamage;
        if (D.setup[i].skill!==undefined && D.setup[i].skill.type=="voidmaster") {
        ...
        if (buff.counter>0&&i==0&&finalDamage>0) {
            if (retturn.other===undefined) retturn.other=getTurnData(D.setup.length,A.setup.length);
            if(A.setup[0].id <= -2 && A.setup[0].skill.type == "antireflect" && A.setup.length > 0) {
                var skillVal = A.setup[0].skill.value;
                if (A.setup[0].prom>=5) skillVal = skillVal + promoData[-2*1-A.setup[0].id].skill;
                var antivalue = finalDamage*buff.counter*(skillVal*Math.min(99,A.setup[0].lvl)/A.setup[0].skill.target);
                retturn.other.atk.flatAoe[0]+=Math.round(finalDamage*buff.counter-antivalue);
            } else retturn.other.atk.flatAoe[0]+=Math.round(finalDamage*buff.counter);
        }
        if (buff.counterFlat>0&&i==0&&finalDamage>0) {
            ...
            if(A.setup[0].id <= -2 && A.setup[0].skill.type == "antireflect" && A.setup.length > 0) {
                var skillVal = A.setup[0].skill.value;
                if (A.setup[0].prom>=5) skillVal = skillVal + promoData[-2*1-A.setup[0].id].skill;
                var antivalue = buff.counterFlat*(skillVal*Math.min(99,A.setup[0].lvl)/A.setup[0].skill.target);
                retturn.other.atk.flatAoe[0]+=Math.round(buff.counterFlat-antivalue);
            } else retturn.other.atk.flatAoe[0]+=Math.round(buff.counterFlat);
        }

        if (buff.counterAoe>0&&i==0&&finalDamage>0) {
            if (retturn.other===undefined) retturn.other=getTurnData(D.setup.length,A.setup.length);
            for (var j=0; j<retturn.other.atk.flatAoe.length; ++j) {
                if(j<A.setup.length && A.setup[j].id <= -2 && A.setup[j].skill.type == "antireflect") {
                    var skillVal = A.setup[j].skill.value;
                    if (A.setup[j].prom>=5) skillVal = skillVal + promoData[-2*1-A.setup[j].id].skill;
                    var antivalue = finalDamage*buff.counterAoe*(skillVal*Math.min(99,A.setup[j].lvl)/A.setup[j].skill.target);
                    retturn.other.atk.flatAoe[j]+=Math.max(1,Math.round(finalDamage*buff.counterAoe-antivalue));
                } else retturn.other.atk.flatAoe[j]+=Math.max(1,Math.round(finalDamage*buff.counterAoe));
            }
        }
            ...
            if (killUnit) {
                ...
                if (D.setup[i].skill!==undefined) {
                    ...
                    if (D.setup[i].skill.type=="explosion") {
                        ...
                        if (retturn.other===undefined) retturn.other=getTurnData(D.setup.length,A.setup.length);
                        if (retturn.other.atk.flatAoe.length !== 0) {
                            retturn.other.atk.flatAoe[0]+=Math.round(skillVal*D.setup[i].payback);
                        }
                    } else if (D.setup[i].skill.type=="fromdeath") {
                        if (retturn.other===undefined) retturn.other=getTurnData(D.setup.length,A.setup.length);
...
function killUnits (A,side,turn) {
    var any=false;
    var kills=[];
    for (var i=A.setup.length-1;i>=0;--i) {
        if (A.setup[i].hp<=0) {
