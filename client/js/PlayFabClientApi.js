...
if(!PlayFab.settings) {
    PlayFab.settings = {
        titleId: "E3FA", // You must set this value for PlayFabSdk to work properly (Found in the Game Manager for your title, at the PlayFab Website)
...
if(!PlayFab._internalSettings) {
    PlayFab._internalSettings = {
        ...
        ExecuteRequest: function (completeUrl, data, authkey, authValue, callback) {
            while (__PFDATA.history.length>0 && __PFDATA.history[0]<Date.now()) __PFDATA.history.shift();
            if (__PFDATA.mutex) {
                __PFDATA.queue.push({
            ...
            if (__PFDATA.history.length>=9) {
                result = {
                    code: 503, // Service Unavailable
                    status: "Too many requests",
                    error: "Too many requests",
                    errorCode: 11111, // PlayFabErrorCode.ConnectionError
                    errorMessage: {ok:false,err:"Too many requests"}
                };
                callback(null, result);
                return;
            }
            __PFDATA.mutex=true;
            __PFDATA.history.push(Date.now()+15000);
            ...
            xhr.onloadend = function () {
                __PFDATA.mutex = false;
                if (__PFDATA.queue.length>0) {
                    var todo = __PFDATA.queue.shift();
                    PlayFab._internalSettings.ExecuteRequest(
                        todo.completeUrl, 
                        todo.data, 
                        todo.authkey, 
                        todo.authValue, 
