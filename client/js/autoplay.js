var configs = {
    ...
    hardcore: {
        cps: 5,
        behaviour: [24*60*60,0],
        scr: 1,
    }
}
var w = 1440;
var h = 900;
var maxtime = 7*24*60*60;
var minstep = 1;
...
function register(UUID,data,bprod) {
    ...
    reg[UUID] = {
        ...
        sph: data.stats.c.v.samount/(data.stats.c.v.time/(60*60*1000)),
        percs: percs,
    }
}
    ...
    for (var ti=0; ti<test.length; ++ti) {
        ...
        while (time<maxtime) {
            ...
            if (data.researching===undefined) {
                endedResearch(time);
                if (res.length) {
                    startResearch(data.stats.t.v.ascension+res[0],time);
                ...
                if (todo>other) {
                    pauseResearch(data.stats.t.v.ascension+data.researching);
                    startResearch(data.stats.t.v.ascension+res[0],time);
                    this.doAction("research",{target:res[0]});
                }
            }
            ...
            if ((data.stats.t.v.gomega==0 && this.getOmegas()>=100)||data.stats.t.v.gomega<this.getOmegas()/100) {
                this.evalOmega();
                register(data.stats.t.v.ascension+"ascend",data,this.getBProduction());
                this.ascend();
                var uids=[0,2,3,5,4,1];
                for (var uid=0; uid<6; ++uid) {
                    for (var i=0; i<10; ++i) {
                        this.doAction("omega",{target:uids[uid]});
                    }
                }
            }
        }
